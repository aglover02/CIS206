...
- Data type validation and range validation (#1).
- Assertions to confirm calculated values are reasonable (#1).
- Exception handling with try/except blocks (#2).
- Nested if statements used within validation (#3).
"""
...
def get_user_input():
    ...
    except Exception as e:  # catch-all (#2)
...
def calculate_bmi(weight_lbs, height_feet, height_inches):
    ...
    # Assertion to ensure BMI is realistic (#1)
...
def main():
    ...
    except AssertionError as ae:  # specific exception handling (#2)
        print(f"Assertion failed: {ae}")
    except Exception as e:  # catch-all safety net (#2)
