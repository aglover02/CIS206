"""
BMI Calculator (Imperial to Metric) with Validation, Error Handling, Looping, and BMI Table.
...
- When the user quits, a BMI table is displayed with:
    - Rows for weights from 100 to 250 pounds in 10-pound increments.
    - Columns for heights from 58 to 76 inches in 2-inch increments.
"""
...
def display_bmi_table():
    """
    Display a BMI table for a range of weights and heights.
    ...
    Notes:
        - The table is displayed only after the user quits the main program loop.

    Returns:
        None
    """
    heights = list(range(58, 77, 2))  # 58–76 inches, 2-inch increments
    weights = list(range(100, 251, 10))  # 100–250 pounds, 10-pound increments

    # Print header
    print("\nBMI Table")
    print("Weights ↓ | Heights (inches) →")
    header = "Weight | " + " ".join(f"{h:6}" for h in heights)
    print(header)
    print("-" * len(header))
    ...
    for weight in weights:
        row = f"{weight:6} |"
        for height_in in heights:
            feet = height_in // 12
            inches = height_in % 12
            bmi_value = calculate_bmi(weight, feet, inches)
            row += f"{bmi_value:6.1f}"
        print(row)
    print("-" * len(header))
...
def main():
    ...
    After quitting, the BMI table is displayed.
    ...
    # Show BMI table only when user quits
    display_bmi_table()
